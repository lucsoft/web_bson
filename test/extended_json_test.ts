...
const getDoc = () => {
  ...
  doc = {
    ...
    dbRef: new DBRef("tests", new Int32(1) as any, "test"),
...
Deno.test(
  ...
  () => {
    ...
    const json = {
      ...
      date: { $date: { $numberLong: "1488372056737" } },
      code: {
        $code: "function() {}",
        $scope: { a: { $numberInt: "1" } },
      },
      dbRef: { $ref: "tests", $id: { $numberInt: "1" }, $db: "test" },
      decimal: { $numberDecimal: "100" },
      ...
      timestamp: { $timestamp: { t: 0, i: 1000 } },
      int32Number: { $numberInt: "300" },
      doubleNumber: { $numberDouble: "200.2" },
      longNumberIntFit: { $numberLong: "7036874417766400" },
      doubleNumberIntFit: { $numberLong: "19007199250000000" },
    };

    assertEquals(
      json,
      JSON.parse(EJSON.stringify(getDoc(), { relaxed: false })),
    );
  },
);
...
Deno.test(
  ...
  () => {
    ...
    "dd";
    let doc1 = EJSON.parse(EJSON.stringify(getDoc())) as any;
    ...
    doc1 = EJSON.parse(EJSON.stringify(getDoc()), { relaxed: false });
...
Deno.test(
  ...
  () => {
    ...
    // Serialize the document
    const text = EJSON.stringify(doc1, { relaxed: false });
...
Deno.test(
  ...
  () => {
    let serialized = EJSON.stringify(new ObjectId("591801a468f9e7024b6235ea"), {
      relaxed: false,
    });
