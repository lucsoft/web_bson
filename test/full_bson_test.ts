...
Deno.test("[Full BSON] Should Correctly Deserialize object", () => {
  ...
  const object = deserialize(new Uint8Array(bytes));
...
Deno.test(
  ...
  () => {
    ...
    const k = new Uint8Array(15);

    k.set(te.encode("hello"), 0);
    k[6] = 0x06;
    k.set(te.encode("world"), 7);

    const v = new Uint8Array(65801);
    for (let i = 0; i < 65801; i++) v[i] = 1;
    v[0] = 0x0a;
    const doc: Document = {};
    doc[td.decode(k)] = td.decode(v);
